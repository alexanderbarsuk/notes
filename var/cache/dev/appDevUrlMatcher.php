    ...
    public function match($pathinfo)
    {
        ...
        if (0 === strpos($pathinfo, '/note')) {
            ...
            if (preg_match('#^/note/(?P<id>[^/]++)$#s', $pathinfo, $matches)) {
                if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'HEAD'));
                    goto not_note_get_one;
                }

                return $this->mergeDefaults(array_replace($matches, array('_route' => 'note_get_one')), array (  '_controller' => 'AppBundle\\Controller\\NoteController::getOneAction',));
            }
            not_note_get_one:
